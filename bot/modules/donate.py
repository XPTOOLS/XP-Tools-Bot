...
def get_donation_buttons(amount: int = 5):
    ...
    if amount == 5:
        ...
        buttons.button(f"{amount} ⭐️", callback_data=f"gift_{amount}")
        buttons.button("+5", callback_data=f"increment_gift_{amount}")
        return buttons.build_menu(b_cols=3)
        
async def generate_invoice(chat_id: int, user_id: int, amount: int, bot: Bot, message_id: int):
...
class DonateCallbackFilter(BaseFilter):
    async def __call__(self, callback_query: CallbackQuery):
        return callback_query.data and (
            callback_query.data.startswith(("gift_", "increment_gift_", "decrement_gift_", "refund_")) or
            callback_query.data == "show_donate_options"
        )
        
@dp.message(Command(commands=["donate", "gift"], prefix="".join(BotCommands)))
@new_task
@SmartDefender
async def donate_command(message: Message, bot: Bot):
    ...
    try:
        ...
        if len(args) == 0:
            ...
            await generate_invoice(message.chat.id, message.from_user.id, amount, bot, message_id)
        else:
            await send_message(
                chat_id=message.chat.id,
        ...
        await send_message(
            chat_id=message.chat.id,
            text="<b>❌ Sorry Bro! Donation System Error</b>",
            parse_mode=ParseMode.HTML
        )
...
async def handle_donate_callback(callback_query: CallbackQuery, bot: Bot):
    ...
    try:
        if data.startswith("gift_"):
            ...
            await generate_invoice(chat_id, user_id, quantity, bot, message_id)
